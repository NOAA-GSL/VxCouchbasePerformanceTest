...
to qualify header predicate values in a subselect, using an inclusive range for fcst_valid_beg iso values,
and to to further qualify the data portion with a set of fcst_leads. This test has a date range that matches the cached sql test case t_1_matchcached_sql.sh to make it possible to compare results. This test is for Couchbase.
PEOF
